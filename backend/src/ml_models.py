...
def compute_arabic_ai_heuristics(text):
    ...
    if wc >= 35 and formal_root_hits >= 7 and cv < 0.52 and human_hits == 0 and ai_structural_markers > 0:
    ...
    elif a13_hits >= 3 and human_hits == 0 and wc >= 35:
        score = max(score, 0.76)

    score = max(0.05, min(0.99, score * 1.25))
...
class TextDetectorModel:
    ...
    def init_transformer(self):
        ...
        try:
            ...
            for model_name in candidates:
                try:
                    logger.info(f"Loading strong English AI-text detector: {model_name}")
                    self.tokenizer = AutoTokenizer.from_pretrained(model_name, local_files_only=not allow_download)
                    ...
                    labels = {int(k): str(v).lower() for k, v in self.transformer_model.config.id2label.items()}
                    ai_idx = None
                    for idx, lab in labels.items():
                        if any(t in lab for t in ("machine", "generated", "fake", "gpt", "ai", "bot", "synthetic", "chatgpt")):
                            ai_idx = idx
                            break
                    if ai_idx is None:
                    ...
                    logger.info(f"Loaded strong AI-text detector {model_name} (ai_label_index={ai_idx}, labels={labels})")
                    return
                except Exception as e:
                    logger.warning(f"Could not load {model_name}: {e}")
                    self.tokenizer = None
                    self.transformer_model = None
            logger.info("No strong transformer detector available; falling back to statistical ensemble.")
        except Exception as e:
    ...
    def init_perplexity_detector(self):
        ...
        try:
            ...
            allow_download = os.environ.get("ALLOW_MODEL_DOWNLOADS", "0") == "1"
            self.perplexity_tokenizer = AutoTokenizer.from_pretrained(name, local_files_only=not allow_download)
            self.perplexity_model = AutoModelForCausalLM.from_pretrained(name, local_files_only=not allow_download).to(self.device)
    ...
    def compute_perplexity(self, text):
        ...
        try:
            ...
            enc = tok(text, return_tensors="pt", truncation=True, max_length=1024).to(self.device)
    ...
    def compute_heuristics(self, text):
        ...
        # Modern-AI combined fingerprint (catches GPT-4 / Claude / Gemini text that
        # deliberately avoids the obvious "delve / tapestry" tells).
        # Signal: polished, impersonal, low-burstiness prose -> highly predictable.
        if wc >= 40 and contraction_rate == 0.0:
            first_person = sum(1 for w in words if w in ("i", "we", "my", "me", "our", "us", "you", "your"))
            exclamations = text.count("!")
            avg_len = sum(len(w) for w in words) / wc if wc else 0
            if first_person == 0 and exclamations == 0 and avg_len >= 5.0 and cv < 0.55:
                # Impersonal, formal, uniform prose -> likely AI
                final = max(final, 0.72)
            if first_person == 0 and avg_len >= 5.3 and cv < 0.45:
                # Even stronger: long words, no personal voice, machine-uniform rhythm
                final = max(final, 0.82)
    ...
    def predict(self, text):
        ...
        # Independent-signal agreement: if BOTH the language-model perplexity and the
        # stylometric heuristics lean AI while the transformer is uncertain, promote
        # the score. This catches modern AI prose the HC3-trained head under-flags.
        if ppl_score is not None and ppl_score >= 0.55 and stat_prob >= 0.55:
            final_prob = max(final_prob, 0.62)

        # Confidence anchoring: when the strong transformer is confident, hold the
        # fused score above/below a sensible bound so weak/legacy heads cannot hide
        # a clearly machine-generated (or clearly human) passage.
        if transformer_prob is not None:
            if transformer_prob >= 0.85:
                final_prob = max(final_prob, 0.82)
            elif transformer_prob >= 0.70:
                final_prob = max(final_prob, 0.66)
            elif transformer_prob <= 0.12:
                final_prob = min(final_prob, 0.16)
            elif transformer_prob <= 0.25:
...
class AdvancedTextEngine:
    ...
    def compute_readability(text):
        ...
        fk_grade = 0.39 * (words / sentences) + 11.8 * (syllables / max(1, words)) - 15.59
...
class AdvancedImageEngine:
    def __init__(self):
        try:
            ...
            import torchvision.models as models
            self.vit = models.vit_b_16(weights=models.ViT_B_16_Weights.DEFAULT if hasattr(models, 'ViT_B_16_Weights') else True)
