...
def compute_advanced_image_forensics(img_pil):
    ...
    try:
        ...
        edge_flat_noise_ratio = float(edge_noise / max(flat_noise, 0.001))
        ...
        v_other = np.delete(vdiff, np.arange(7, vdiff.shape[1], 8), axis=1).mean() if vdiff.shape[1] > 8 else 1.0
        h_other = np.delete(hdiff, np.arange(7, hdiff.shape[0], 8), axis=0).mean() if hdiff.shape[0] > 8 else 1.0
        ...
        return {
            ...
            "real_weight": round(float(real_weight), 3),
    ...
    try:
        ...
        return {
            ...
            "metrics": {
                ...
                "personal_specificity": int(personal_specificity),
                "anchored_specificity": int(anchored_specificity),
                "surface_human_noise": int(surface_human_noise),
                "emoji_noise": int(emoji_noise),
                ...
                "arabic_ai_hits": int(arabic_ai_hits),
                "arabic_transition_hits": int(arabic_transition_hits),
                "arabic_human_hits": int(arabic_human_hits),
                "narrative_hits": int(narrative_hits),
                "scenic_hits": int(scenic_hits),
                "simple_narrative_verb_hits": int(simple_narrative_verb_hits),
                "business_phrase_hits": int(business_phrase_hits),
                "business_word_hits": int(business_word_hits)
            }
        }
    except Exception as e:
...
def run_text_detection(text, requested_language="auto"):
    ...
    surface_human_noise = int(advanced_metrics.get("surface_human_noise", 0) or 0)
    anchored_specificity = int(advanced_metrics.get("anchored_specificity", advanced_metrics.get("personal_specificity", 0)) or 0)
    humanized_ai_suspected = (
        surface_human_noise >= 1 and
        anchored_specificity <= 1 and
        advanced_ai_weight >= max(1.4, advanced_human_weight + 0.2)
    )
    ...
    if human_hits and not humanized_ai_suspected and advanced_ai_weight < max(3.2, advanced_human_weight + 1.0):
    ...
    if advanced_text.get("success"):
        ...
        if ai_floor and advanced_ai_weight >= advanced_human_weight + 0.35:
            final_prob = max(final_prob, ai_floor)
        if human_cap and not (advanced_ai_weight >= advanced_human_weight + 0.8 and advanced_ai_weight >= 3.0):
            final_prob = min(final_prob, human_cap)

    if humanized_ai_suspected:
        final_prob = max(final_prob, 0.78 if advanced_ai_weight >= 2.2 else 0.64)

    if indicator_hits == 0 and human_hits >= 2 and advanced_ai_weight < 2.2 and not humanized_ai_suspected:
        final_prob = min(final_prob, 0.46)
    elif indicator_hits == 0 and len(words) < 50 and advanced_ai_weight < 1.8 and not humanized_ai_suspected:
    ...
    prediction = "AI-GENERATED" if final_prob >= 0.5 else "Human"
    ...
    for sentence in split_text_sentences(text):
        ...
        elif sent_advanced.get("success") and sent_advanced.get("ai_probability_floor", 0) >= 0.60:
            sent_prob = max(sent_prob, sent_advanced.get("ai_probability_floor", 0.60))
        elif sent_human_hits >= 1 and sent_advanced.get("ai_weight", 0) < 2.0:
        ...
        sent_pred = "AI-GENERATED" if sent_prob >= 0.5 else "HUMAN"
    ...
    features = {
        ...
        "humanization_attempt": "high" if humanized_ai_suspected or (advanced_ai_weight >= 4.0 and advanced_human_weight >= 1.0) else ("medium" if (surface_human_noise >= 1 or (final_prob > 0.4 and final_prob < 0.6)) else "low"),
        ...
        "advanced_text_reasons": ", ".join(advanced_text.get("top_ai_reasons", [])[:5]) if advanced_text.get("success") else "not available",
        "human_text_reasons": ", ".join(advanced_text.get("top_human_reasons", [])[:5]) if advanced_text.get("success") else "not available",
...
def run_image_detection(img_pil, file_size, original_name, raw_bytes=None):
    ...
    if forensics.get("success", False):
        ...
        if advanced_res.get("success"):
            ...
            if advanced_ai_weight >= 4.2 and advanced_strong_votes >= 3 and advanced_ai_weight >= advanced_real_weight + 1.4:
                heur_score += 0.58
                strong_synthetic_evidence = True
                reasons.append(
            ...
            elif advanced_ai_weight >= 3.0 and advanced_strong_votes >= 2 and advanced_ai_weight >= advanced_real_weight + 1.0:
                heur_score += 0.34
                strong_synthetic_evidence = True
                reasons.append(
            ...
            elif advanced_real_votes >= 3 and advanced_ai_weight < 3.2:
        ...
        if metadata_unverified:
            ...
            if advanced_ai_weight >= 3.0 and advanced_ai_weight >= advanced_real_weight + 1.0:
                synthetic_votes += 2
                review_flags.append(f"advanced forensic synthetic votes on unverified image (weight={advanced_ai_weight:.2f})")
            elif advanced_ai_weight >= 2.6 and advanced_ai_weight >= advanced_real_weight + 0.5:
                synthetic_votes += 2
        ...
        if metadata_unverified:
            ...
            elif advanced_ai_weight >= 1.8 and advanced_ai_weight >= advanced_real_weight - 0.1:
        ...
        if (
            ...
            dct_energy < 12.0 and
            fft_peak_z < 4.4 and
            advanced_ai_weight < 3.0 and
            not (metadata_unverified and ai_dimension_hint)
            and (
                not metadata_unverified or
                (benford_verified and advanced_real_weight >= 3.0 and synthetic_votes == 0)
            )
        ):
            natural_capture_evidence = True
        ...
        if camera_hint_found:
            ...
            if advanced_ai_weight >= 4.2 and advanced_ai_weight >= advanced_real_weight + 1.6:
                camera_artifacts.append(f"advanced synthetic forensic ensemble weight={advanced_ai_weight}")
            if not camera_workflow_verified and advanced_ai_weight >= 3.0 and advanced_ai_weight >= advanced_real_weight + 0.8:
    ...
    run_neural_model = not ai_tag_found and not (
        camera_workflow_verified and
        not strong_synthetic_evidence and
        heur_score < 0.55
    )
    if run_neural_model:
    ...
    if (
        ...
        (advanced_ai_weight >= advanced_real_weight - 0.2 or ai_dimension_hint or synthetic_votes >= 1)
    ):
    ...
    if (
        ...
        advanced_ai_weight < 2.8 and
        advanced_ai_weight <= advanced_real_weight + 0.2 and
        heur_score < 0.35 and
    ...
    if (
        ...
        (natural_capture_evidence or advanced_real_weight >= 2.0 or benford_verified) and
        heur_score < 0.50
    ):
    ...
    return {
        ...
        "features": {
            ...
            "I01_Hand_Finger_Analysis": "Anomalous Texture" if heur_score >= 0.5 and not is_lossy_smoothed else "Pass",
            "I02_Face_Analysis": "Synthetic Features Detected" if any("Face detected" in r for r in reasons) else "Pass",
            "I03_Text_Rendering": "Pass",
            "I04_Lighting_Analysis": "Anomalous Depth" if forensics.get("ela_ratio", 1.0) > 4.0 else "Consistent",
            ...
            "I08_Advanced_Forensic_Ensemble": (
                ...
                f"signals={len(advanced_res.get('signals', []))}"
            ...
            "decision_path": "; ".join(reasons[:8]) if reasons else "No decisive AI artifacts detected",
...
def run_video_detection(video_path, file_size, original_name):
    ...
    return {
        ...
        "features": {
            ...
            "V02_Facial_Landmark": "Pass",
            "V03_Lip_Sync": "Pass",
            "V04_Deepfake": "Anomaly Detected" if heur_score >= 0.35 else "Pass",
            "V05_Blinking": "Pass",
            "V06_Motion_Authenticity": f"Temporal Flow Variance: {avg_temporal_diff:.3f}",
            "V07_Audio_Authenticity": "Spectral Scan Complete",
            ...
            "suspected_generator": suspected,
            "decision_path": "; ".join(reasons[:8]) if reasons else "No decisive AI video artifacts detected",
            "review_flags": "strict video provenance enabled"
        }
    }
