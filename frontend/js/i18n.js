...
(function () {
  ...
  const dict = {
    en: {
      ...
      detector: {
        ...
        subtitle: 'Unescapable intelligence. Drop your content below and let the neural engine dissect the truth.',
        ...
        errors: {
          minText: 'Please enter at least 50 characters for meaningful analysis.',
          ...
          limit: 'You have reached your {plan} plan limit of {limit} scans. Redirecting to upgrade...'
        },
        sample: 'In the rapidly evolving landscape of modern technology, leveraging synergistic paradigms is crucial. To delve into the myriad of possibilities, we must foster a holistic ecosystem that underscores pivotal transformative capabilities.',
        loadingLogs: [
      ...
      auth: {
        ...
        devCodeSent: '[DEV MODE] Code sent! Check developer console or use: {code}',
        failedDispatch: 'Failed to dispatch verification code. Please try again.',
        ...
        errors: {
          ...
          legacy: 'This account was created with a legacy password. Please contact support to migrate to password-less login.'
        }
      },
      payment: {
      ...
      staticPages: {
        ...
        notFoundContent: 'We are actively working on bringing you this content. Please check back soon.',
        docs: {
          title: 'Documentation',
    ...
    ar: {
      ...
      detector: {
        ...
        errors: {
          ...
          limit: 'لقد وصلت إلى حد خطة {plan}: {limit} فحوصات. سيتم تحويلك للترقية...'
        },
        sample: 'من الجدير بالذكر أن التحول الرقمي أصبح عاملا محوريا في تطوير المؤسسات الحديثة. علاوة على ذلك، تسهم هذه التقنيات في تعزيز الكفاءة وتحسين جودة الخدمات. وفي الختام، يمكن القول إن الذكاء الاصطناعي يمثل فرصة استراتيجية لبناء مستقبل أكثر ابتكارا واستدامة.',
        loadingLogs: [
      ...
      auth: {
        ...
        devCodeSent: '[وضع التطوير] تم إرسال الرمز! تحقق من وحدة التحكم أو استخدم: {code}',
        ...
        errors: {
          ...
          legacy: 'تم إنشاء هذا الحساب بكلمة مرور قديمة. يرجى التواصل مع الدعم لترحيله إلى تسجيل الدخول بدون كلمة مرور.'
        }
      },
      payment: {
      ...
      staticPages: {
        ...
        careers: {
          ...
          content: 'نبحث دائما عن مواهب مميزة للانضمام إلى فريقنا. لا توجد وظائف معلنة حاليا، لكن يمكنك العودة لاحقا مع استمرار نمو الفريق.'
        },
        blog: {
          title: 'المدونة',
        ...
        privacy: {
          ...
          content: 'نأخذ خصوصيتك بجدية. نجمع الحد الأدنى من البيانات اللازمة لتقديم خدماتنا، ونعمل على إعداد سياسة خصوصية شاملة توضح ممارسات البيانات.'
        },
        terms: {
          title: 'شروط الخدمة',
        ...
        cookies: {
          ...
          content: 'نستخدم ملفات تعريف الارتباط لتحسين تجربة التصفح وتحليل الزيارات. ستتوفر هنا تفاصيل إدارة التفضيلات والملفات المستخدمة.'
        },
        security: {
          title: 'الأمان',
          subtitle: 'حماية بمستوى المؤسسات.',
          content: 'نستخدم ممارسات أمنية رائدة لحماية بياناتك وتفاعلاتك مع المنصة. يجري إعداد وثائق الأمان وشهادات الامتثال.'
        }
      }
    }
  };
  ...
  function format(value, vars) {
    ...
    return value.replace(/\{(\w+)\}/g, (_, key) => vars && vars[key] !== undefined ? vars[key] : `{${key}}`);
  }
  ...
  function translateTextNodes(root) {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        ...
        if (parent.closest('script, style, code, pre, textarea, [data-no-i18n], #provenance-code-section')) {
  ...
  function translatePlaceholders(root) {
    root.querySelectorAll('input[placeholder], textarea[placeholder]').forEach(el => {
      if (!el.dataset.i18nSourcePlaceholder) {
        el.dataset.i18nSourcePlaceholder = el.getAttribute('placeholder') || '';
      }
      const source = el.dataset.i18nSourcePlaceholder;
      el.setAttribute('placeholder', isArabic() ? (placeholderAr[source] || source) : source);
    });
  }
  ...
  function injectLanguageControls() {
    ...
    if (!document.querySelector('[data-language-switcher="mobile"]')) {
      ...
      if (mobileList) {
        const btn = makeLangButton('text-white hover:text-[var(--accent-1)] text-left font-black');
        btn.dataset.languageSwitcher = 'mobile';
        mobileList.appendChild(btn);
      }
    }

    if (!document.querySelector('[data-language-switcher="floating"]') && !document.getElementById('nav-login-btn')) {
      const btn = makeLangButton('fixed top-4 right-4 z-[200] px-4 py-2 bg-black/70 border border-white/15 text-white hover:text-[var(--accent-1)] hover:border-[var(--accent-1)] transition-colors font-black text-xs tracking-wider rounded-lg backdrop-blur');
