...
window._initSigninModal = function (modalRoot) {
  ...
  if (typeof firebase !== "undefined") {
    ...
    if (user && authCard) {
      const alreadySignedIn = escapeHtml(
        authTr("auth.alreadySignedIn", null, "Already Signed In"),
      );
      const loggedInAs = escapeHtml(
        authTr("auth.loggedInAs", null, "You are logged in as"),
      );
      const goDetector = escapeHtml(
        authTr("auth.goDetector", null, "Go to Detector"),
      );
  ...
  async function sendOTPCode(email) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || data.details || "Failed to send OTP email",
        );
      }

      successMsg.textContent = authTr(
        "auth.codeSent",
        { email },
      ...
      errorMsg.textContent = authTr(
        "auth.emailJsError",
        { detail: errMsgDetail },
        `Email Sending Error: ${errMsgDetail}. Make sure SMTP is configured in backend.`,
      );
      showAlert(errorMsg);
      return false;
    }
    ...
    switch (code) {
      ...
      case "auth/operation-not-allowed":
        return authTr(
          ...
          null,
          "Authentication method is currently disabled.",
        );
      default:
        return authTr(
          "auth.errors.default",
