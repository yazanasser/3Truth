...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (auroraBlobs.length > 0) {
    ...
    function updateAurora() {
      driftTime += 0.01;
      const autoX = Math.sin(driftTime * 0.85) * 34 + Math.sin(driftTime * 0.26) * 18;
      const autoY = Math.cos(driftTime * 0.72) * 26 + Math.sin(driftTime * 0.34) * 14;
  ...
  function processChunked(items, processItem, delay = 15) {
    let i = 0;
    const idleCb = window.requestIdleCallback || ((cb) => setTimeout(cb, delay));
    function next() {
  ...
  function createParticles() {
    ...
    const panels = Array.from(document.querySelectorAll('.horizontal-panel, .panel-particles'));
    let currentIndex = 0;

    function processNextPanel() {
      if (currentIndex >= panels.length) return;
      const panel = panels[currentIndex++];
      let pCont = panel.classList.contains('panel-particles') ? panel : panel.querySelector('.panel-particles');
      if (!pCont) {
        pCont = document.createElement('div');
        pCont.className = 'panel-particles absolute inset-0 z-0 overflow-hidden pointer-events-none opacity-80';
  ...
  if (typeof firebase !== 'undefined' && firebase.auth) {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      if (user) {
        ...
        if (desktopLoginBtn) {
          desktopLoginBtn.textContent = window._3truthI18n ? window._3truthI18n.t('ui.signOut', null, 'SIGN OUT') : 'SIGN OUT';
        ...
        if (sidebarLoginLink) {
          sidebarLoginLink.textContent = window._3truthI18n ? window._3truthI18n.t('ui.signOut', null, 'SIGN OUT') : 'SIGN OUT';
